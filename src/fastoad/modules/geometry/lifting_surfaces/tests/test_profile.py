...
def test_set_points(point_set):
    ...
    assert_allclose(0., np.min(relative_thickness['x']), rtol=1e-4)
    assert_allclose(1., np.max(relative_thickness['x']), rtol=1e-4)
    assert_allclose(0.10070, np.max(relative_thickness['thickness']), rtol=1e-4)


    # import matplotlib.pyplot as plt
    # plt.axes(aspect='equal')
    # plt.plot(profile.get_upper_side_points()['x'], profile.get_upper_side_points()['z'], 'o-')
    # plt.plot(profile.get_lower_side_points()['x'], profile.get_lower_side_points()['z'], 'o-')
    # plt.plot(profile.get_mean_line()['x'], profile.get_mean_line()['z'], 'o-')
    # profile.twist_angle = 30.
    # plt.plot(profile.get_upper_side_points()['x'], profile.get_upper_side_points()['z'])
    # plt.plot(profile.get_lower_side_points()['x'], profile.get_lower_side_points()['z'])
    # plt.plot(profile.get_mean_line()['x'], profile.get_mean_line()['z'])
    # plt.show()
...
def test_twist_angle(point_set):
    ...
    # Check base twist angle
    leading_edge = profile.get_upper_side().iloc[0, :]
    trailing_edge = profile.get_upper_side().iloc[-1, :]
    current_twist_angle = np.arctan2(trailing_edge['z'] - leading_edge['z'],
                                     trailing_edge['x'] - leading_edge['x'])
    assert_allclose(base_twist_angle, current_twist_angle)

    # Changing twist angle 1
    profile.twist_angle = 5.
    leading_edge = profile.get_mean_line().iloc[0, :]
    trailing_edge = profile.get_mean_line().iloc[-1, :]
    current_twist_angle = np.arctan2(trailing_edge['z'] - leading_edge['z'],
                                     trailing_edge['x'] - leading_edge['x'])
    assert_allclose(base_twist_angle - np.radians(5.), current_twist_angle)

    # Changing twist angle 2
    profile.twist_angle = -15.
    leading_edge = profile.get_lower_side().iloc[0, :]
    trailing_edge = profile.get_lower_side().iloc[-1, :]
    current_twist_angle = np.arctan2(trailing_edge['z'] - leading_edge['z'],
                                     trailing_edge['x'] - leading_edge['x'])
    assert_allclose(base_twist_angle - np.radians(-15.), current_twist_angle)
