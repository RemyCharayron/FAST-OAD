...
class LiftingSurface(ComponentGeometry):
    ...
    def __init__(self):
        ...
        self.mac_position = Coordinates2D(0., 0.)
        self.planform_area: float = 0.
    ...
    def compute_planform_area(self) -> float:
        ...
        """
        area = pd.DataFrame([self._compute_trapezoid_area(section1, section2) for
    ...
    def compute_mean_aerodynamic_chord(self):
        ...
        """
        local_mac_profiles = pd.Series([self._compute_trapezoid_mac(section1, section2) for
                                        section1, section2 in
                                        zip(self.sections[:-1], self.sections[1:])])
        area = pd.Series([self._compute_trapezoid_area(section1, section2) for
                          section1, section2 in
                          zip(self.sections[:-1], self.sections[1:])])
        ...
        self.mac_position = Coordinates2D(np.dot(local_mac_x, area) / np.sum(area),
                                          np.dot(local_mac_y, area) / np.sum(area))
    ...
    def _compute_trapezoid_mac(root_profile: Profile, tip_profile: Profile) -> Profile:
        ...
        mac.planform_position = Coordinates2D(root_x + (tip_x - root_x) * coeff,
                                              root_y + (tip_y - root_y) * coeff)
