...
def _rotate_2d_array(array: Union[np.ndarray, pd.DataFrame], theta: float) \
        -> Union[np.ndarray, pd.DataFrame]:
    theta_rad = np.radians(theta)
    rot_matrix = np.array([[np.cos(theta_rad), -np.sin(theta_rad)],
                           [np.sin(theta_rad), np.cos(theta_rad)]])
    return np.matmul(array, rot_matrix)
...
class Profile:
    ...
    def set_points(self, x: Sequence, z: Sequence,
        ...
        z_ref = np.interp(0., self._rel_mean_line_and_thickness[X],
                          self._rel_mean_line_and_thickness[Z])
    ...
    def get_relative_thickness(self) -> pd.DataFrame:
        ...
        return self._rel_mean_line_and_thickness[[X, THICKNESS]] \
               * [1., self.max_relative_thickness] + [0.25, 0.]
    ...
    def _compute_mean_line_and_thickness(self, upper_side_points, lower_side_points) \
            -> Tuple[float, float]:
        """
        ...
        """
        x = lower_side_points[X].append(
            upper_side_points[X]).drop_duplicates().sort_values()

        interp_lower = interp1d(lower_side_points[X], lower_side_points[Z],
                                kind='quadratic')
        interp_upper = interp1d(upper_side_points[X], upper_side_points[Z],
                                kind='quadratic')
        z_sides = pd.DataFrame({'z_lower': interp_lower(x),
                                'z_upper': interp_upper(x)}
                               )
        ...
        self._rel_mean_line_and_thickness = pd.DataFrame({X: x / chord_length,
                                                          Z: z / chord_length,
    ...
    @staticmethod
    def _create_upper_lower_sides(x: Sequence, z: Sequence) \
        ...
        side1 = pd.DataFrame({X: x[i1:i2 + 1],
                              Z: z[i1:i2 + 1]})
        side2_1 = pd.DataFrame({X: x[i2:],
                                Z: z[i2:]})
        side2_2 = pd.DataFrame({X: x[:i1 + 1],
                                Z: z[:i1 + 1]})
